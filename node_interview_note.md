...
> How many types of Authentication Tokens
 ...
 5. MAC Tokens: Similar to bearer tokens, but include a cryptographic signature for each request.
 6. OAuth 2.0 Tokens: OAuth 2.0 is an authorisation framework that uses several types of tokens to grant access to resources.
...
2. Server Verifies Credentials: Validates the email and password against the database. If the credentials are correct, it generates a new authentication token (JWT) for the client. 
...
4. Client Stores the Token: The client stores the token securely in an HTTP-only cookie or local storage.


> JWT Authentication

We are actually working on a JWT token for authentication.
JSON Web Token (JWT) is an open standard that defines a compact and self-contained way for securely transmitting information between parties as a JSON object. This information can be verified and trusted because it is digitally signed. JWTs can be signed using a secret (with the HMAC algorithm) or a public/private key pair, such as RSA or ECDSA.
...
const users = []; // Example users array (replace with DB in production)
...
> What is the best approch to use payload in JWT?
